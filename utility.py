...
import json,datetime,asyncio,random,pytz,os
...
ALLOWED_USER=int(config['allowed_user_id'])
PREMIUM_ROLE_ID=int(config['Premium_Role_Id'])if config['Premium_Role_Id'].strip()else 0
REVIEW_CHANNEL_ID=int(config['Review_channel_id'])if config['Review_channel_id'].strip()else 0
DROP_CHANNEL_ID=int(config['Drop_Channel_Id'])if config['Drop_Channel_Id'].strip()else 0
BOOSTER_ROLE_ID=int(config.get('Booster_Role_Id',0))if str(config.get('Booster_Role_Id','')).strip()else 0
...
def load_stats():
    ...
    except:return{'total':0,'sum':0,'last_five':[]}
def save_stats(stats):
    with open(STATS_FILE,'w')as f:json.dump(stats,f,indent=2)
stats=load_stats()

DB_FILE='boosters.json'
def load_boosters():
    try:
        with open(DB_FILE)as f:return json.load(f)
    except:return{}
def save_boosters(data):
    with open(DB_FILE,'w')as f:json.dump(data,f,indent=2)
boosters_db=load_boosters()
...
def human_delta(dt):
    ...
    delta=now-dt
    days=delta.days
    if days>=365:return f"{days//365} year{'s'if days//365!=1 else''} ago"
    if days>=30:return f"{days//30} month{'s'if days//30!=1 else''} ago"
    if days>=7:return f"{days//7} week{'s'if days//7!=1 else''} ago"
    return f"{days} day{'s'if days!=1 else''} ago"

@bot.event
async def on_ready():
    await bot.change_presence(activity=discord.CustomActivity(name='Flow Cloud Ultimate Generator'))
    await bot.tree.sync()
    check_growth.start()
    print('Bot online')
...
async def start_purge_wave(guild):
    ...
    if purge_active:await perform_purge(guild,channel)
...
async def perform_purge(guild,channel):
    ...
    for reaction in msg.reactions:
        if str(reaction.emoji)=='✅':
            async for user in reaction.users():reacted.add(user.id)
    to_kick=[]
    for member in guild.members:
        if member.bot:continue
        if member.guild_permissions.administrator or member.guild_permissions.manage_guild:continue
        if member.id not in reacted:to_kick.append(member)
    for member in to_kick:
        try:await member.kick(reason='Purge Wave - inactive')
        except:pass
    await channel.send(f'🧹 Purge complete. {len(to_kick)} members removed.')
    purge_active=False

@bot.tree.command(name='setpremiumrole')
async def setpremiumrole(interaction,role:discord.Role):
    if interaction.user.id!=ALLOWED_USER:return await interaction.response.send_message('❌ Unauthorized',ephemeral=True)
    global config
...
@bot.tree.command(name='setboosterrole')
async def setboosterrole(interaction,role:discord.Role):
    if interaction.user.id!=ALLOWED_USER:return await interaction.response.send_message('❌ Unauthorized',ephemeral=True)
    global config
    config['Booster_Role_Id']=str(role.id)
    save_config(config)
    await interaction.response.send_message(f'✅ Booster role set to {role.mention}',ephemeral=True)

@bot.tree.command(name='setreview')
async def setreview(interaction,channel:discord.TextChannel):
    if interaction.user.id!=ALLOWED_USER:return await interaction.response.send_message('❌ Unauthorized',ephemeral=True)
    global config
...
async def setdrop(interaction,channel:discord.TextChannel):
    if interaction.user.id!=ALLOWED_USER:return await interaction.response.send_message('❌ Unauthorized',ephemeral=True)
    global config
...
async def purgewave(interaction):
    if interaction.guild_id!=GUILD_ID:return await interaction.response.send_message('This command only works in Flow Cloud.',ephemeral=True)
    if interaction.user.id!=ALLOWED_USER:return await interaction.response.send_message('❌ Unauthorized',ephemeral=True)
    await interaction.response.send_message('✅ Purge wave started',ephemeral=True)
    await start_purge_wave(interaction.guild)

@bot.tree.command(name='purgecancel')
async def purgecancel(interaction):
    if interaction.guild_id!=GUILD_ID:return await interaction.response.send_message('This command only works in Flow Cloud.',ephemeral=True)
    if interaction.user.id!=ALLOWED_USER:return await interaction.response.send_message('❌ Unauthorized',ephemeral=True)
    global purge_active,purge_channel_id
    if not purge_active:return await interaction.response.send_message('No active purge wave',ephemeral=True)
...
async def drop(interaction,file:discord.Attachment):
    if interaction.user.id!=ALLOWED_USER:return await interaction.response.send_message('❌ Unauthorized',ephemeral=True)
    channel=bot.get_channel(DROP_CHANNEL_ID)
    if not channel:return await interaction.response.send_message('Drop channel not found',ephemeral=True)
    if not file.filename.endswith('.txt'):return await interaction.response.send_message('❌ Only .txt files are allowed',ephemeral=True)
    await interaction.response.send_message('✅ Drop sent',ephemeral=True)
    est=pytz.timezone('US/Eastern')
    now=datetime.datetime.now(est).strftime('%B %d, %Y %I:%M %p')
    embed=discord.Embed(title='🔥 New Drop Alert',description=f'{file.filename} from the Flow Cloud Staff Team\n\nDownload the file to access content inside',color=discord.Color.cyan(),timestamp=datetime.datetime.utcnow())
...
async def si(ctx):
    ...
    embed=discord.Embed(title=g.name,description=f"{desc}\n\n**General Info**\n**Name:** {g.name}\n**Server ID:** {g.id}\n**Owner:** {owner}\n**Created:** {created}",color=discord.Color.cyan())
    embed.add_field(name="Members & Roles",value=f"**Members:** {humans}\n**Bots:** {bots}\n**Total:** {g.member_count}\n**Roles:** {len(g.roles)}",inline=False)
    embed.add_field(name="Boost Status",value=f"**Server Boost Level:** {lvl}\n**Amount of Boosts:** {boosts}",inline=False)
...
async def si_slash(interaction):
    g=interaction.guild
    desc=g.description or"No description set"
    owner=g.owner.mention if g.owner else"Unknown"
    created=f"{human_delta(g.created_at)} ({g.created_at.strftime('%B %d, %Y at %I:%M %p')})"
...
async def shop(ctx,channel:discord.TextChannel):
    if ctx.author.id!=ALLOWED_USER:return await ctx.send('❌ Unauthorized',delete_after=5)
    guild=ctx.guild
    icon_url=guild.icon.url if guild.icon else''
    embed=discord.Embed(title='Flow Cloud Shop System',description='Welcome to Flow Cloud Shop\n\nHere you can purchase stuff like our discord bots premium Gen stock server access even accounts or methods\n\n**What we offer**\nPremium Generator Access\nStock Server Access\nBot Source Codes\n\n**Important Note:** We are not negotiating our prices we list no exceptions all prices and sales are final\n\n**We take these payment methods**\n💵 Cash App\n🍏 Apple Pay\n💳 PayPal\n🪙 ETH\n🪙 LTC\n🪙 BTC\n💸 Venmo',color=discord.Color.cyan())
    embed.set_thumbnail(url=icon_url)
    embed.set_footer(text='Flow Cloud Shop System')

    class ShopDropdown(discord.ui.Select):
        def __init__(self):
            options=[discord.SelectOption(label='Premium Generator Access',emoji='🔑',description='Buy premium generator access'),discord.SelectOption(label='Stock Server Access',emoji='📊',description='Buy stock server access'),discord.SelectOption(label='Bot Source Codes',emoji='🤖',description='Buy bot source codes')]
            super().__init__(placeholder='Choose what u want to buy',options=options)
        async def callback(self,interaction):
            if self.values[0]=='Premium Generator Access':await interaction.response.send_message('🔑 You can buy access to our <#1450598555617202339> channel for these prices\n1D = $0.25\n3D = $0.50\n7D/1W = $1\n1 Month = $3\nLifetime = $5',ephemeral=True)
            elif self.values[0]=='Stock Server Access':await interaction.response.send_message('📊 Stock server access for\n1D = $1\n3D = $2\n7D = $5\n1 Month = $8\nLifetime = $10',ephemeral=True)
            elif self.values[0]=='Bot Source Codes':await interaction.response.send_message('🤖 Here you can buy our bots source codes that we made ourself for cheap prices\n\nFlow Cloud Generator Source Code $20\nFlow Cloud Utility Source Code $5\nFlow Cloud Pass Change Bot $25',ephemeral=True)
...
async def premium_give(interaction,user:discord.Member,duration:str):
    ...
    embed=discord.Embed(title='✅ Premium Granted',description=f'{user.mention} has received the {role.mention} role for {duration}',color=discord.Color.green())
    await interaction.response.send_message(embed=embed)
...
async def on_member_update(before,after):
    if BOOSTER_ROLE_ID==0:return
    guild=after.guild
    role=guild.get_role(BOOSTER_ROLE_ID)
    if not role:return
    boosted=after.premium_since is not None and before.premium_since is None
    unboosted=after.premium_since is None and before.premium_since is not None
    if boosted:
        await after.add_roles(role)
        boosters_db[str(after.id)]={'role_id':BOOSTER_ROLE_ID,'boosted_at':datetime.datetime.utcnow().isoformat()}
        save_boosters(boosters_db)
    elif unboosted:
        await after.remove_roles(role)
        boosters_db.pop(str(after.id),None)
        save_boosters(boosters_db)
